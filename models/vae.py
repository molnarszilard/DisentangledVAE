...
class VAE(pl.LightningModule):
    ...
    def criterion(X, X_hat, mean, std):
        ...
        pcdgt = X.view(-1,3,128*128)
        pcdgt = nn.functional.interpolate(pcdgt, size=(2048), mode='linear')
        pcd_reco = X_hat.view(-1,3,128*128)
        pcd_reco = nn.functional.interpolate(pcd_reco, size=(2048), mode='linear')
        loss_cd = torch.sum(reconstruction_loss(pcdgt.permute(0, 2, 1) + 0.5,
